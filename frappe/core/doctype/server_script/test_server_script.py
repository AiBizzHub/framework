...
from frappe.frappeclient import AiBizzAppClient, AiBizzAppException
...
"""
	...
	def test_server_script_rate_limiting(self):
		...
		client = AiBizzAppClient(site)

		# Exhaust rate limit
		for _ in range(5):
			client.get_api(script1.api_method)

		self.assertRaises(AiBizzAppException, client.get_api, script1.api_method)

		# Exhaust rate limit
		for _ in range(5):
			client.get_api(script2.api_method)

		self.assertRaises(AiBizzAppException, client.get_api, script2.api_method)
