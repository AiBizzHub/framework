...
from frappe.email.frappemail import AiBizzAppMail
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		...
		service: DF.Literal[
			"", "AiBizzApp Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.service == "AiBizzApp Mail":
			self.use_imap = 0
		...
		if (
			...
			and not self.service == "AiBizzApp Mail"
		):
	...
	def get_account_details_from_site_config(cls):
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "AiBizzApp"},
	...
	def _frappe_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return AiBizzAppMail(self.frappe_mail_site, self.email_id, access_token=access_token)

			frappe.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					frappe.bold(self.email_account_name)
				),
				title=_("AiBizzApp Mail OAuth Error"),
			)
		else:
			return AiBizzAppMail(
