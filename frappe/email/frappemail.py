...
from frappe.frappeclient import AiBizzAppClient, AiBizzAppOAuth2Client
from frappe.utils import convert_utc_to_system_timezone, get_datetime, get_datetime_str, get_system_timezone


class AiBizzAppMail:
	"""Class to interact with the AiBizzApp Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> AiBizzAppClient | AiBizzAppOAuth2Client:
		"""Returns a AiBizzAppClient or AiBizzAppOAuth2Client instance."""
		...
		client = (
			AiBizzAppOAuth2Client(url=site, access_token=access_token)
			if access_token
			else AiBizzAppClient(url=site, api_key=api_key, api_secret=api_secret)
		)
	...
	def request(
		...
		"""Makes a request to the AiBizzApp Mail API."""
	...
	def send_raw(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an email using the AiBizzApp Mail API."""
	...
	def send_newsletter(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an newsletter using the AiBizzApp Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, list[str] | str]:
		"""Pulls emails from the mailbox using the AiBizzApp Mail API."""
