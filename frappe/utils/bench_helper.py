...
def AiBizzAppClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	AiBizzAppClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="bench")
...
def get_app_groups() -> dict[str, click.Group | click.Command]:
	...
	return dict(
		frappe=click.group(
			name="frappe", commands=commands, cls=AiBizzAppClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=AiBizzAppClickWrapper(click.Group, handle_exception)
		)(app_group)
