"""
AiBizzApp Testing Module

This module provides a comprehensive framework for running tests in AiBizzApp applications.
...
- TestRunner: Main class for running test suites with additional AiBizzApp-specific functionality
- TestResult: Custom test result class for improved output formatting and logging
- discover_all_tests: Function to discover all tests in specified AiBizzApp apps
...
- Integration with AiBizzApp's hooks and test record creation system

Usage:
This module is typically used by AiBizzApp's CLI commands for running tests, but can also
...
from frappe.utils.logger import create_handler as createAiBizzAppFileHandler
...
handlers = createAiBizzAppFileHandler(__name__)
