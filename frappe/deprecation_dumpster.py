...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the AiBizzApp framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The AiBizzApp version in which it will be removed (its "graduation" to the great codebase in the sky)
...
class AiBizzAppDeprecationWarning(Warning):
	...
...
try:
	...
	def _deprecated(message: str, category=AiBizzAppDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecation_warning(marked: str, graduation: str, msg: str):
	...
	warnings.warn(
		...
		+ colorize(f"{msg}\n", Color.YELLOW),
		category=AiBizzAppDeprecationWarning,
		stacklevel=2,
	)
...
def get_tests_CompatAiBizzAppTestCase():
	...
	class AiBizzAppTestCase(unittest.TestCase):
		"""Base test class for AiBizzApp tests.
		...
		@deprecated(
			"frappe.tests.utils.AiBizzAppTestCase",
			"2024-20-08",
			"v17",
			"Import `frappe.tests.UnitTestCase` or `frappe.tests.IntegrationTestCase` respectively instead of `frappe.tests.utils.AiBizzAppTestCase` - also see wiki for more info: https://github.com/frappe/frappe/wiki#testing-guide",
		)
		...
		# --- AiBizzApp Framework specific assertions
	...
	return AiBizzAppTestCase


# remove alongside get_tests_CompatAiBizzAppTestCase
def get_compat_frappe_test_case_preparation(cfg):
	...
	class AiBizzAppTestCasePreparation(IntegrationTestPreparation):
	...
	return AiBizzAppTestCasePreparation(cfg)
