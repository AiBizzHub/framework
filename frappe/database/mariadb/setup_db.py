...
def check_compatible_versions():
	try:
		...
		if version_tuple < (10, 6):
			click.secho(
				f"Warning: MariaDB version {version} is less than 10.6 which is not supported by AiBizzApp",
				fg="yellow",
			)
		elif version_tuple >= (10, 9):
			click.secho(
				f"Warning: MariaDB version {version} is more than 10.8 which is not yet tested with AiBizzApp Framework.",
				fg="yellow",
			)
	except Exception:
		click.secho(
